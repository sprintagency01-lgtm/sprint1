...
### Corregido (en el mismo día)

- **Drift gordo en el prompt remoto del agente pelu_demo (`agent_4201...`)**. Detectado tras una llamada real: Ana hacía 2 preguntas por turno y pedía nombre antes de consultar disponibilidad, violando la jerarquía sagrada del prompt. Comparación local vs remoto: local `ana_prompt_new.txt` 12.500 chars con todas las secciones críticas (`UNA pregunta por turno`, `Nombre al FINAL`, `Flujo RESERVA`, `REFRESH_BLOCK`); remoto solo 3.741 chars y CERO de esas secciones. Prompt remoto era una versión recortada antigua, probablemente editada en la UI de ElevenLabs y nunca re-sincronizada con `refresh_agent_prompt.py`. Backup del drift en `docs/snapshots/agent_pelu_demo_prompt_drifted_2026-05-03T200500Z.txt`. Sincronizado con `ELEVENLABS_AGENT_ID=agent_4201... python3 scripts/refresh_agent_prompt.py` → PATCH 200 OK, remoto ahora 12.474 chars. Smoke post-sync: primer mensaje "Perfecto, ¿qué servicio te hacemos?" — UNA pregunta, sin enumerar servicios, conforme al canónico.
