// Pantalla Conversaciones · compacta, colapsa lista→detalle
...
function ScreenConversaciones() {
  ...
  const list = tab === 'whatsapp' ? CONVERSACIONES_WA : CONVERSACIONES_VOZ;
  ...
  const sel = list.find(c => c.id === openId);

  return (
    <>
      <div className="mb-4">
        <h1 className="text-xl md:text-2xl font-bold tracking-tight">Conversaciones</h1>
        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Lo que el bot ha hablado con tus clientes</p>
      </div>

      <div className="border-b border-slate-200 dark:border-slate-800 mb-3 flex gap-5">
        {[['whatsapp','WhatsApp',ICONS.whatsapp,CONVERSACIONES_WA.length],['voz','Voz',ICONS.voz,CONVERSACIONES_VOZ.length]].map(([k,l,ic,n])=>(
      ...
      <Card className="overflow-hidden">
        <ul className="divide-y divide-slate-100 dark:divide-slate-800">
                ...
                <button onClick={()=>setOpenId(isOpen?null:c.id)}
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="text-[11px] text-slate-400 shrink-0 tabular-nums">{c.ultimoAt.split(' ')[1]}</div>
                    </div>
                    <div className="flex items-center gap-1.5">
                      ...
                      {tab==='voz' && <span className="shrink-0 text-[10px] text-slate-400 tabular-nums">{c.duracion}</span>}
                    </div>
                  </div>
                ...
                {isOpen && (
                  <div className="px-4 pb-4 bg-slate-50 dark:bg-slate-950/50 fade-in">
                    ...
                    <div className="space-y-2 pt-2">
                      {(tab==='whatsapp' ? c.mensajes : c.turnos).map((m,i)=>(
          ...
          {!filtered.length && <li className="p-10 text-center text-sm text-slate-400">Sin resultados.</li>}
        </ul>
      </Card>
    </>
  );
...
window.ScreenConversaciones = ScreenConversaciones;
